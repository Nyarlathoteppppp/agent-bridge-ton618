...
`/Users/ywbw/cowork` is the fixed collaboration sandbox. Prefer task artifacts under:
...
- You should not edit files during plan review unless Claude explicitly asks you to implement a patch.

When Claude sends a plan review request:
1. Review the plan, not just the code.
2. Look for missing constraints, unsafe assumptions, wrong files, untested behavior, and rollback risk.
3. Give a direct verdict: approve, approve with changes, or block.
4. Include concrete file paths, commands, or tests when relevant.
5. Do not rewrite the whole plan unless the current plan is materially unsafe.
